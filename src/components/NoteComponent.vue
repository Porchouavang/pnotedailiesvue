                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="form-group ml-2 mb-3">
                                            ...
                                            <textarea name="" id="" cols="30" v-model="content" rows="10" class="form-control"
                                                placeholder="ເນື້ອໃນຕ່າງໆ"></textarea>
                                        </div>
                                        <div class="form-group ml-2 mb-3">
                                ...
                                <div class="modal-body">
                                    <div class="row">
                                        <div class="form-group ml-3 mb-3">
                                            <label for="exampleInputEmail1">ເນື້ອໃນ:<span
                                                    class="text-danger">*</span></label>
                                            <textarea name="" id="" cols="30" rows="10" class="form-control" required v-model="updatecontent"></textarea>
                                        </div>
                                        <div class="form-group ml-3 mb-3">
                                            <label for="exampleInputEmail1">ໂມດູລ:<span
                                                    class="text-danger">*</span></label>
                                                    <select name="" id="" v-model="updatemodulename" class="form-control">
                                                <option v-for="module_s in modules_s" :key="module_s.id" :value="module_s.id">
...
export default {
    ...
    methods: {
        ...
        async ShowNotes() {
            try {
                const response = await axios.get('https://nodejsbackenddailies.onrender.com/api/select-note');
        ...
        async CreateNote() {
            try {
                const response = await axios.post('https://nodejsbackenddailies.onrender.com/api/create-note', {
    ...
    async DeleteNote(Note_ID) {
      try {
        const response = await axios.delete(`https://nodejsbackenddailies.onrender.com/api/delete-note/${Note_ID}`);
        console.log(response.data);
        Swal.fire({
          title: 'ລົບລ້າງ!',
    ...
    async SearchNote() {
      ...
      fetch(`https://nodejsbackenddailies.onrender.com/api/search-note?${params.toString()}`)
    ...
    async SearchNoteByModule() {
      ...
      fetch(`https://nodejsbackenddailies.onrender.com/api/search-note-by-module?${params.toString()}`)
        .then((response) => response.json())
        .then((data) => {
          this.note_s = data;
          this.none_data = data.length === 0;
    ...
    async ShowNoteItem(Note_ID){
      ...
      try {
        const response = await axios.get(`https://nodejsbackenddailies.onrender.com/api/note/${Note_ID}`);
    ...
    async UpdateNote() {
      try {
        const response = await axios.put(`https://nodejsbackenddailies.onrender.com/api/update-note/${this.Note_ID}`, {
